...
class GPTConfig:
    ...
    num_query_heads: int = compute_heads(
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.ffn_multipliers, Number):
            ...
            self.ffn_multipliers = [
                self.ffn_multipliers] * self.num_transformer_layers
        elif (
    ...
    def __call__(
        ...
        if use_kv_cache:
            if past_key_value is not None:
                ...
                queries = self.pos_embedding(
                    queries, offset=past_keys.shape[2])
    ...
    def reset_parameters(self, model_config: GPTConfig) -> None:
        ...
        for module in self.modules():
            if isinstance(module, (nn.Linear)):
                ...
                module.apply(nn.init.normal(std=std),
                             filter_fn=name_filter("weight"))
                module.apply(nn.init.constant(0.0),
                             filter_fn=name_filter("bias"))
            elif isinstance(module, (nn.Embedding)):
                std = model_dim**-0.5
                module.apply(nn.init.normal(std),
                             filter_fn=name_filter("weight"))
            elif isinstance(module, (nn.RMSNorm, nn.LayerNorm)):
                module.apply(nn.init.constant(1.0),
                             filter_fn=name_filter("weight"))
                module.apply(nn.init.constant(0.0),
                             filter_fn=name_filter("bias"))
