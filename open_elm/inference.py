...
import open_elm
from numpy import array as nparray  # TODO: how to omit numpy requirement?
...
def generate_token(
    ...
    def sample(logits, key=False):
        if sampling_temperature == 0:
            return mx.argmax(logits, axis=-1)
        # couldn't get ctypes.c_uint32 to work
        if key:
            # not used
            # for truly deterministic outputs
            # can also try:
            # from ctypes import c_uint32
            key = mx.array(nparray([-5, 9], dtype="uint32"))
